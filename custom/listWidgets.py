...
class UsedListWidget(MyListWidget):
    ...
    def show_attr(self):
        ...
        if type(item) in items:
            ...
            cw = self.mainwindow.stackedWidget.currentWidget()
            if cw is not None and hasattr(cw, 'update_params'):
                cw.update_params(param)  # 更新对应的table
                try:
                    self.mainwindow.dock_attr.show()
                except Exception:
                    pass
            else:
                # 如果没有可用的 widget，确保属性面板不显示以避免 NoneType 错误
                try:
                    self.mainwindow.dock_attr.hide()
                except Exception:
                    pass
